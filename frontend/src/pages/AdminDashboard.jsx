  ...
  const handleSaveService = async (form) => {
    ...
    try {
      ...
      if (d.success) {
        ...
        showToast(d.message || "Service not found", "error");
        if (r.status === 404) {
          setServiceModal(null);
          fetchServices();
        }
  ...
  const handleSaveProf = async (form) => {
    ...
    try {
      ...
      if (d.success) {
        ...
        showToast(d.message || "Professional not found", "error");
        if (r.status === 404) {
          setProfModal(null);
          fetchProfessionals();
        }
            ...
            {activeSection === "emergencies" && (
              <div style={{ background: "#fff", borderRadius: "16px", boxShadow: "0 1px 3px rgba(0,0,0,0.07)", overflow: "hidden" }}>
                <div style={{ padding: "18px 24px", borderBottom: "1px solid #f3f4f6", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <button onClick={fetchEmergencies} style={{ padding: "6px 12px", borderRadius: "8px", border: "1px solid #e5e7eb", background: "#fff", cursor: "pointer", fontSize: "0.8rem", fontWeight: 600 }}>
                    Refresh
                  </button>
                </div>
                <div style={{ overflowX: "auto" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    <thead>
                      <tr style={{ background: "#fef2f2" }}>
                        {["User", "Category & Severity", "Description", "Location", "Specialist", "Status", "Time", "Action"].map(h => (
                    ...
                    <tbody>
                      {emergencies.map(e => {
                        const statusColors = {
                          Dispatched: { bg: "#fef3c7", color: "#d97706" },
                          OnTheWay:   { bg: "#dbeafe", color: "#2563eb" },
                          Arrived:    { bg: "#ede9fe", color: "#7c3aed" },
                          Resolved:   { bg: "#dcfce7", color: "#16a34a" },
                          Cancelled:  { bg: "#f3f4f6", color: "#6b7280" }
                        };
                        const sc = statusColors[e.status] || { bg: "#fef2f2", color: "#ef4444" };
                        return (
                          <tr key={e._id} onMouseEnter={el => el.currentTarget.style.background = "#fffbfb"} onMouseLeave={el => el.currentTarget.style.background = "transparent"} style={{ borderBottom: "1px solid #f9fafb" }}>
                            <td style={{ padding: "14px 16px" }}>
                              <div style={{ fontWeight: 600, fontSize: "0.85rem" }}>{e.user?.name || "Customer"}</div>
                              <div style={{ fontSize: "0.74rem", color: "#9ca3af" }}>{e.user?.email || e.contactNumber}</div>
                            </td>
                            <td style={{ padding: "14px 16px" }}>
                              <div style={{ fontWeight: 700, fontSize: "0.85rem" }}>{e.category || e.issueType || "Emergency"}</div>
                              <span style={{ fontSize: "0.7rem", fontWeight: 700, padding: "2px 6px", borderRadius: "4px", background: "#fef2f2", color: "#dc2626" }}>
                                {e.severity || "High"}
                              </span>
                            </td>
                            ...
                            <td style={{ padding: "14px 16px", fontSize: "0.82rem", color: "#6b7280", maxWidth: "160px" }}>
                              {e.address || e.location || "—"}
                            </td>
                            <td style={{ padding: "14px 16px", fontSize: "0.83rem", fontWeight: 600 }}>
                              {e.assignedProfessional?.name || <span style={{ color: "#d97706", fontSize: "0.78rem" }}>Auto-assigning...</span>}
                            </td>
                            <td style={{ padding: "14px 16px" }}>
                              <span style={{ padding: "3px 10px", borderRadius: "20px", fontSize: "0.74rem", fontWeight: 700, background: sc.bg, color: sc.color }}>
                                {e.status}
                              </span>
                            </td>
                            <td style={{ padding: "14px 16px", fontSize: "0.78rem", color: "#9ca3af" }}>
                            ...
                            <td style={{ padding: "14px 16px" }}>
                              <select
                                ...
                                onChange={async (event) => {
                                  ...
                                  try {
                                    const r = await fetch(`${BASE}/emergencies/${e._id}/status`, {
                                      method: "PUT",
                                      headers: jsonHeaders,
                                ...
                                style={{ padding: "6px 10px", borderRadius: "8px", border: "1px solid #e5e7eb", fontSize: "0.78rem", cursor: "pointer", fontWeight: 600 }}
                              >
                      ...
                      {emergencies.length === 0 && (
                        <tr><td colSpan={8} style={{ textAlign: "center", padding: "40px", color: "#9ca3af" }}>No emergency requests found.</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
